...
export default function GoogleBranchMap({ latitude, longitude, radius, onLocationChange }: GoogleBranchMapProps) {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      try {
        ...
        map.addListener('click', (e: any) => {
          if (e.latLng && mountedRef.current) {
            marker.setPosition(e.latLng);
            circle.setCenter(e.latLng);
            onLocationChange(e.latLng.lat(), e.latLng.lng());
            infoWindow.open(map, marker);
            setTimeout(() => {
              if (mountedRef.current) infoWindow.close();
            }, 1500);
          }
        });
    ...
    const loadGoogleMaps = () => {
      ...
      // Timeout
      setTimeout(() => {
        if (loading && !window.google?.maps && mountedRef.current) {
          window.__googleMapsLoading = false;
    ...
    // Cleanup - SAFE from React Strict Mode
    return () => {
      ...
      setTimeout(() => {
        try {
          ...
          // Don't destroy the map, just clear the reference
  ...
  useEffect(() => {
    if (window.google?.maps && markerRef.current && circleRef.current && mapRef.current && mountedRef.current) {
      try {
