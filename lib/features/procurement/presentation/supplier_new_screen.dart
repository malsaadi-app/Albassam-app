...
import '../data/supplier_requests_service.dart';
...
class _SupplierNewScreenState extends State<SupplierNewScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // If backend blocks direct supplier creation for this role, we fallback to supplier-request workflow.
      ...
      try {
        ...
        final reqSvc = SupplierRequestsService(api);
        await reqSvc.create(
          name: _name.trim(),
          contactPerson: _contactPerson,
          email: _email,
      ...
      if (mounted) {
        final msg = isAr ? 'تم الإرسال' : 'Submitted';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isAr ? 'مورد جديد' : 'New Supplier')),
      body: Form(
        key: _formKey,
        child: ListView(
