...
console.log('\n🔍 استخراج الموظفين الذين لديهم مشاكل (من الملف المصحح)...\n');
...
try {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    data.forEach((row, idx) => {
      ...
      // Problem 1: "لغات أجنبية أخرى" (should only be in girls branches now)
      ...
      if (!school || school === 'NULL' || school === 'غير محدد') {
        hasProblem = true;
        problemType = 'بيانات ناقصة';
        problemDescription = problemDescription ? problemDescription + ' | المرحلة مفقودة' : 'المرحلة مفقودة أو NULL';
      }
  ...
  // Save
  const outputPath = '/data/.openclaw/workspace/albassam-tasks/data/problematic_employees_updated.xlsx';
  XLSX.writeFile(newWorkbook, outputPath);
  
  console.log('✅ تم إنشاء ملف المشاكل المحدث!\n');
  ...
  // Show breakdown by branch for "لغات أجنبية أخرى"
  if (problemCategories['لغات أجنبية أخرى'].length > 0) {
    console.log('📋 توزيع "لغات أجنبية أخرى" حسب الفرع:\n');
    const byBranch = {};
    problemCategories['لغات أجنبية أخرى'].forEach(emp => {
      const branch = emp['الفرع'];
  ...
  // Save JSON
  fs.writeFileSync(
    '/data/.openclaw/workspace/albassam-tasks/data/problematic_employees_updated.json',
    JSON.stringify({
      summary: summary,
  ...
  console.log('💡 ملاحظة:');
  console.log('   ✅ تم تصحيح: مجمع البسام الأهلية بنين (32 موظف → المواد الأدبية)');
  console.log('   ✅ تم تصحيح: المعاهد (31 موظف → المرحلة الصحيحة)\n');
